...
### High-Level Summary
All core architectural components (patchify, class token, learnable positional embeddings, multi-head self-attention, MLP blocks, residual connections, layer normalization, output head) are implemented manually in PyTorch (no using `timm` ViT directly).  
Best test accuracy achieved: **90.51%** after 200 epochs.
...
- **Longer Schedule (exp-5):** 300 epochs produced marginal ±0.1% improvement; sweet spot ~200–250 epochs.

### Key Implementation Notes
- Manual patch extraction using `unfold` or reshape (no high-level ViT wrappers).  
...
5. Run SAM 2 multimask inference; fallback to SAM (ViT-H) if SAM 2 load fails.  
6. Score candidate masks by `(CLIPSeg overlap × SAM score)` and select best.  
7. Visualize: original → heatmap → initial mask → refined masks → final overlay.

### Bonus Video Extension
- Extract frames (stride/fps sampling).  
...
| `SegmentationEvaluator` | IoU, Dice, Precision, Recall, F1, coverage, basic ground-truth synthesis |
...
- Overlap weighting:  
  ```python
  combined_score = overlap_with_clipseg_heatmap * sam_score
  ```  
...
- Graceful logging when no detection (suggests threshold adjustment).

### Evaluation Components
- Real-world images: dog, cat, car, person, flower.  
- Synthetic shapes: circle, square, triangle, diamond, hexagon.  
- Multi-object video (three simultaneous shapes).  
- Threshold sweep: 0.10–0.65 (step 0.05).  
- Plot suite summarizing success, coverage, timing, and thresholds.
...
5. Synthetic video test:
   ...
   video_segmenter.process_video(
       "synthetic_circle.mp4",
       "green circle",
       max_frames=10,
       threshold=0.3
   )
...
### Limitations
- CLIPSeg can miss fine-grained / multi-instance objects.  
- Single-prompt, single-object focus (no simultaneous multi-object segmentation).  
- External checkpoint URLs are single points of failure.  
- Overlap scoring heuristic not learned; could mis-rank regions.  
- Comprehensive suite runtime (10–15+ min) is high for fast iteration.  
- No caching of downloads / intermediate features.  
- Binary threshold not adaptive; no CRF/boundary refinement.  
- High memory use on large images or videos.

### Future Improvements
- Integrate GroundingDINO / GLIP for stronger boxes.  
- Negative prompts & multi-object parsing.  
- Optical flow or tracker-based temporal smoothing.  
- Learned fusion or lightweight refinement head.  
...
| Ambiguous prompt | Encourage more specific text |

### Summary Statement
This segmentation framework fuses **CLIPSeg grounding** with **SAM 2 refinement** and extends to video through re-grounded temporal propagation, delivering a flexible and analyzable platform for text-driven segmentation research.
...
| Basic Tests | `run_comprehensive_tests()` |
| Full Evaluation Suite | `run_comprehensive_evaluation()` |
